...
def fdm_create_acess_policy(host,token,payload,parent_id):
    '''
    This is a POST request to create a access policy.
    '''
    headers = {
        "Content-Type": "application/json",
        "Accept": "application/json",
    ...
    try:
        request = requests.post("https://{}:{}/api/fdm/v2/object/networks".format(host, FDM_PORT,parent_id),
...
def fdm_create_access_policy(host,token,payload,parent_id):
    ...
    try:
        request = requests.post("https://{}:{}/api/fdm/v2/policy/accesspolicies/{}/accessrules".format(host, FDM_PORT,parent_id),
                    json=payload, headers=headers, verify=False)
        return request.json()
    except:
        raise
